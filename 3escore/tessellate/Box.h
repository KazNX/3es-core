...
namespace tes::box
{
/// Tessellate a solid box mesh. The mesh is considered solid in that it is not transparent (as
...
void TES_CORE_API solid(std::vector<Vector3f> &vertices, std::vector<unsigned> &indices,
                        std::vector<Vector3f> &normals, const Vector3f &scale = Vector3f(1.0f));
