...
///
/// This abstracts away the details of the shader to a common interface since Magnum shaders often
/// use different function names for similar operations.
...
public:
  ...
  /// @return
  [[nodiscard]] virtual Feature features() const = 0;

  /// Check the supported features.
  ///
  /// Can be used to check a single feature, or a feature set, in which case all features must be
  /// supported.
  ///
  ...
  [[nodiscard]] bool supportsFeatures(Feature feature_flags) const;
  ...
  [[nodiscard]] bool supportsFeatureAny(Feature feature_flags) const;

  /// Access the underlying shader.
  /// @return A pointer to the Magnum Graphics shader.
  [[nodiscard]] virtual std::shared_ptr<Magnum::GL::AbstractShaderProgram> shader() const = 0;

  /// Set the projection matrix for the next @c draw() call. This is only the projection matrix and
  /// must be combined with the view and model matrices either in the shader or for the shader.
  /// @param matrix The next projection matrix to draw with.
  /// @return @c *this
  virtual Shader &setProjectionMatrix(const Magnum::Matrix4 &matrix) = 0;

  /// Set the view matrix for the next @c draw() call. This is only the view matrix and must be
  /// combined with the projection and model matrices either in the shader or for the shader.
  /// @param matrix The next view matrix to draw with.
  /// @return @c *this
  virtual Shader &setViewMatrix(const Magnum::Matrix4 &matrix) = 0;
  /// Set the model matrix for the next @c draw() call. This is only the model matrix and must be
  /// combined with the projection and view matrices either in the shader or for the shader.
  ...
  /// Sets the draw scale for things which support it. This depends on the shader, but is used for
  /// point rendering size, line width, font size, etc.
  ///
  /// Setting a zero draw scale implies using the "default" draw scale. Negative values yield
  /// undefined behaviour.
  ///
  ...
  ///
  /// May be called multiple times for each frame with only one call to @c setProjectionMatrix() in
  /// between.
  ///
  /// The underlying shader must support instanced rendering.
  ///
  /// @param mesh The mesh to draw.
  /// @param buffer The shape instance buffer or @c Magnum::Matrix4 and @c Magnum::Color4 pairs per
  /// instance.
  /// @param instance_count Number of instances in @p buffer .
  /// @return @c *this
  virtual Shader &draw(Magnum::GL::Mesh &mesh, Magnum::GL::Buffer &buffer,
