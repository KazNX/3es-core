...
public:
  ...
  Bounds(const Magnum::Vector3 &min_ext, const Magnum::Vector3 &max_ext)
    : Super(convert(min_ext), convert(max_ext))
  {}
  Bounds(const Magnum::Vector3 &point)
    : Super(convert(point))
  {}
  Bounds(const tes::Vector3<Magnum::Float> &min_ext, const tes::Vector3<Magnum::Float> &max_ext)
    : Super(min_ext, max_ext)
  {}
  Bounds(const tes::Vector3<Magnum::Float> &point)
    : Super(point)
  {}
  ...
  static Bounds fromCentreHalfExtents(const Magnum::Vector3 &centre,
                                      const Magnum::Vector3 &half_extents)
  {
  ...
  [[nodiscard]] Magnum::Vector3 minimum() const { return convert(Super::minimum()); }
  /// Access the maximum extents.
  /// @return The maximal corder of the bounding box.
  [[nodiscard]] Magnum::Vector3 maximum() const { return convert(Super::maximum()); }

  /// Get the bounds centre point.
  /// @return The bounds centre.
  [[nodiscard]] Magnum::Vector3 centre() const { return convert(Super::centre()); }
  /// Get the bounds half extents, from centre to max.
  /// @return The half extents, centre to max.
  [[nodiscard]] Magnum::Vector3 halfExtents() const { return convert(Super::halfExtents()); }

  /// Expand the bounding box to include @p point.
  /// @param point The point to include.
  void expand(const Magnum::Vector3 &point) { expand(convert(point)); }
  ...
  [[nodiscard]] bool operator==(const Bounds &other) const { return Super::operator==(other); }
...
///
/// Anything which requires a bounds check for rendering can add a bounds entry via @c allocate() ,
/// which is disposed of with @c release() when no longer required. The @c allocate() function
/// returns a @c BoundsId is used to @c update() the bounds, check @c isVisible() or @c release()
/// when done.
///
/// Before rendering, the @c BoundsCuller must have @c cull() called in order to update the
/// visibility of all tracked bounds entries. This requires a @c mark value which identifies the
/// current frame. The particular value is especially important, so long as it changes each frame
/// and has a long period before returning to the same value. During
/// @p cull() each bounds visible bounds entry is stamped with this @p mark value. The same @p mark
/// can later be used to check visibility via @p isVisible() .
...
public:
  ...
  [[nodiscard]] bool isVisible(BoundsId id, unsigned render_mark) const;
  ...
  [[nodiscard]] bool isVisible(BoundsId id) const { return isVisible(id, _last_mark); }

  /// Allocate a new bounds entry with the given bounds.
  /// @param bounds Bounds AABB.
  /// @return The bound entry ID.
  [[nodiscard]] BoundsId allocate(const Bounds &bounds);
